...
export async function groqComplete(params) {
  ...
  for (const key of FALLBACK_KEYS) {
    ...
    try {
      ...
      // Retry with next key on auth, rate-limit, or account restriction errors
      const status = err?.status;
      const isAuthError = status === 401 || status === 403 || msg.includes('401') || msg.includes('403') || msg.includes('unauthorized');
      const isRateLimit = status === 429 || msg.includes('429') || msg.includes('rate_limit');
      const isRestriction = msg.includes('restricted') || msg.includes('organization_restricted');
      
      if (isAuthError || isRateLimit || isRestriction) {
        console.warn(`Groq key ending ...${cleanKey.slice(-6)} failed (Status: ${status || 'N/A'}, Msg: ${isRestriction ? 'restricted' : isRateLimit ? 'rate-limited' : 'auth error'}), trying next key...`);
        continue;
      }
